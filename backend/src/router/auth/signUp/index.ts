...
export const signUpTrpcRoute = trpcLoggedProcedure.input(zSignUpTrpcInput).mutation(async ({ ctx, input }) => {
  ...
  if (exUserWithNick) {
    throw new ExpectedError('USER_ALREADY_EXISTS', 'User with this nick already exists')
  }
  ...
  if (exUserWithEmail) {
    throw new ExpectedError('USER_ALREADY_EXISTS', 'User with this email already exists')
  }
